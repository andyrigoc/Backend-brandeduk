 ...
 * Useful for displaying brand filters in the frontend/admin
 */
router.get('/brands', async (req, res) => {
  try {
    ...
    console.error('[ERROR] Failed to fetch brands:', {
      message: error.message,
      stack: error.stack,
    });
    res.status(500).json({ 
 ...
 * GET /api/products/brands
 * Get all brands with product counts
 * Useful for displaying brand filters in the admin hub
 */
router.get('/brands', async (req, res) => {
  try {
    const query = `
