...
/**
 * Product Pricing State - stores editable pricing data
 */
let productPricingState = {
    markupPercent: 50,
    volumeDiscountTable: 'default',
    discountBaseOnly: false,
    useManualBasePrice: false,
    manualBasePrice: 0,
    sizePrices: {} // { 'XS': { carton: 8.60, pack: 9.40, single: 10.25, manualBase: null }, ... }
};

/**
 * Default Volume Discount Levels
 */
const VOLUME_DISCOUNT_LEVELS = [
    { minQty: 8, discount: 12 },
    { minQty: 24, discount: 20.5 },
    { minQty: 49, discount: 27.5 },
    { minQty: 99, discount: 28 },
    { minQty: 299, discount: 35 }
];
...
function getVolumeDiscount(qty) {
    if (productPricingState.volumeDiscountTable === '' || productPricingState.volumeDiscountTable === 'none') {
        return 0;
    }
    let discount = 0;
...
/**
 * Calculate marked up price from supplier cost
 */
function calculateMarkedUpPrice(supplierCost, qty = 1) {
    const markup = productPricingState.markupPercent / 100;
    let basePrice = supplierCost * (1 + markup);
    
    // Apply volume discount
    const volumeDiscount = getVolumeDiscount(qty);
    if (volumeDiscount > 0) {
        if (productPricingState.discountBaseOnly) {
            // Discount on supplier cost only
            basePrice = supplierCost * (1 - volumeDiscount / 100) * (1 + markup);
        } else {
...
/**
 * PRICING TAB - DecoNetwork Style Complete with Editable Size Matrix
 */
function renderPricingTab(product) {
    // Initialize size pricing from product data
    initializeSizePricing(product);
    
    const supplierCost = product.priceRange?.min || product.minPrice || product.price || 8.60;
    const markupPercent = productPricingState.markupPercent;
    const markupAmount = (supplierCost * markupPercent / 100).toFixed(2);
    const blankProductPrice = productPricingState.useManualBasePrice && productPricingState.manualBasePrice > 0
        ? productPricingState.manualBasePrice.toFixed(2)
        : (parseFloat(supplierCost) + parseFloat(markupAmount)).toFixed(2);
        ...
        <div class="section-body">
            ...
            <div class="pricing-section">
                ...
                <div class="form-group" style="max-width: 300px; margin-top: 16px;">
                    ...
                    <select id="discountTable" onchange="updateVolumeDiscountTable(this.value)">
                        <option value="" ${productPricingState.volumeDiscountTable === '' ? 'selected' : ''}>No discount</option>
                        <option value="default" ${productPricingState.volumeDiscountTable === 'default' ? 'selected' : ''}>Use Default Discount</option>
                        <option value="good" ${productPricingState.volumeDiscountTable === 'good' ? 'selected' : ''}>A good discount</option>
                        <option value="bulk" ${productPricingState.volumeDiscountTable === 'bulk' ? 'selected' : ''}>Bulk buyer discount</option>
                    </select>
                </div>
                
                <div class="checkbox-group" style="margin-top: 12px;">
                    <input type="checkbox" id="discountBaseOnly" ${productPricingState.discountBaseOnly ? 'checked' : ''} onchange="updateDiscountBaseOnly(this.checked)">
            ...
            <div class="pricing-section">
                <div class="pricing-section-header">
                    <div>
                        <h3 class="pricing-section-title">Size Pricing Matrix</h3>
                        <p class="pricing-section-desc">Configure supplier costs and your markup per size. Red prices show your sell price after markup.</p>
                    </div>
                    <button class="btn btn-secondary" onclick="recalculatePricingMatrix()">
                        <i class="fas fa-sync-alt"></i> Recalculate
                    </button>
                </div>
                
                <div class="markup-controls" style="display: flex; gap: 20px; align-items: flex-end; margin: 16px 0; flex-wrap: wrap;">
                    <div class="form-group" style="margin-bottom: 0;">
                        <label>Markup %</label>
                        <div style="display: flex; align-items: center; gap: 8px;">
                            <input type="number" id="globalMarkupPercent" value="${productPricingState.markupPercent}" 
                                   style="width: 80px;" min="0" max="500" step="0.5" 
                                   onchange="updateGlobalMarkup(this.value)">
                            <span>%</span>
                        </div>
                    </div>
                    <div class="form-group" style="margin-bottom: 0;">
                        <label class="checkbox-label" style="display: flex; align-items: center; gap: 8px;">
                            <input type="checkbox" id="useManualBasePrice" ${productPricingState.useManualBasePrice ? 'checked' : ''} 
                                   onchange="toggleManualBasePrice(this.checked)">
                            Override with manual base price
                        </label>
                    </div>
                    <div class="form-group" style="margin-bottom: 0; ${productPricingState.useManualBasePrice ? '' : 'display: none;'}" id="manualBasePriceGroup">
                        <label>Manual Base Price (£)</label>
                        <input type="number" id="manualBasePriceInput" value="${productPricingState.manualBasePrice.toFixed(2)}" 
                               style="width: 100px;" min="0" step="0.01" 
                               onchange="updateManualBasePrice(this.value)">
                    </div>
                </div>
                
                <div class="size-pricing-table-wrapper" style="overflow-x: auto;">
                    ${renderSizePricingMatrix(product)}
                </div>
            </div>
...
/**
 * ===== SIZE PRICING MATRIX FUNCTIONS =====
 */

/**
 * Initialize size pricing from product data
 */
function initializeSizePricing(product) {
    const sizes = product.sizes || product.sizeVariants || ['XS', 'S', 'M', 'L', 'XL', '2XL', '3XL', '4XL', '5XL'];
    const basePrice = product.priceRange?.min || product.minPrice || product.price || 8.60;
    
    // Default size pricing based on DecoNetwork style
    const defaultPrices = {
        'XS': { carton: 8.60, pack: 9.40, single: 10.25, cartonQty: 24, packQty: 6, stock: 0, supplierStock: 3312 },
        'S': { carton: 8.60, pack: 9.40, single: 10.25, cartonQty: 24, packQty: 6, stock: 3177, supplierStock: 13920 },
        'M': { carton: 8.60, pack: 9.40, single: 10.25, cartonQty: 24, packQty: 6, stock: 3322, supplierStock: 3264 },
        'L': { carton: 8.60, pack: 9.40, single: 10.25, cartonQty: 24, packQty: 6, stock: 5801, supplierStock: 11712 },
        'XL': { carton: 8.60, pack: 9.40, single: 10.25, cartonQty: 24, packQty: 6, stock: 2408, supplierStock: 6504 },
        '2XL': { carton: 8.60, pack: 9.40, single: 10.25, cartonQty: 24, packQty: 6, stock: 2068, supplierStock: 3240 },
        '3XL': { carton: 12.30, pack: 13.45, single: 14.70, cartonQty: 24, packQty: 6, stock: 183, supplierStock: 3864 },
        '4XL': { carton: 12.30, pack: 13.45, single: 14.70, cartonQty: 12, packQty: 6, stock: 249, supplierStock: 3660 },
        '5XL': { carton: 12.30, pack: 13.45, single: 14.70, cartonQty: 12, packQty: 6, stock: 112, supplierStock: 1332 }
    };
    
    // Reset if switching products
    if (Object.keys(productPricingState.sizePrices).length === 0 || 
        !productPricingState.sizePrices[sizes[0]]) {
        productPricingState.sizePrices = {};
        
        (Array.isArray(sizes) ? sizes : []).forEach((size, idx) => {
            const sizeKey = typeof size === 'string' ? size : size.name || `Size${idx}`;
            if (defaultPrices[sizeKey]) {
                productPricingState.sizePrices[sizeKey] = { ...defaultPrices[sizeKey], manualBase: null };
            } else {
                // Default for unknown sizes
                productPricingState.sizePrices[sizeKey] = {
                    carton: basePrice,
                    pack: basePrice * 1.1,
                    single: basePrice * 1.2,
                    cartonQty: 24,
                    packQty: 6,
                    stock: 0,
                    supplierStock: 0,
...
/**
 * Render Size Pricing Matrix Table
 */
function renderSizePricingMatrix(product) {
    const sizes = Object.keys(productPricingState.sizePrices);
    if (sizes.length === 0) return '<p class="no-data">No sizes available</p>';
    
    const markup = productPricingState.markupPercent / 100;
    
    return `
        <table class="size-pricing-matrix">
            <thead>
                <tr>
                    <th>Size</th>
                    <th>Carton <span class="qty-note">(Qty)</span></th>
                    <th>Pack <span class="qty-note">(Qty)</span></th>
                    <th>Single <span class="qty-note">(1)</span></th>
                    <th>Stock</th>
                    <th>Supplier Stock<br><small>+4 Days</small></th>
                    <th>Watch</th>
                    <th>More Due</th>
                    <th>Qty</th>
                </tr>
            ...
            <tbody>
                ${sizes.map(size => {
                    const p = productPricingState.sizePrices[size];
                    const cartonSell = calculateMarkedUpPrice(p.carton, p.cartonQty);
                    const packSell = calculateMarkedUpPrice(p.pack, p.packQty);
                    const singleSell = calculateMarkedUpPrice(p.single, 1);
                    
                    return `
                        <tr data-size="${size}">
                            <td class="size-cell"><strong>${size}</strong></td>
                            <td class="price-cell">
                                <div class="price-supplier">
                                    <span class="editable-price" onclick="editSupplierPrice('${size}', 'carton', this)">£${p.carton.toFixed(2)}</span>
                                    <span class="qty-badge">(${p.cartonQty})</span>
                                </div>
                                <div class="price-sell">£${cartonSell.toFixed(2)} *</div>
                            </td>
                            <td class="price-cell">
                                <div class="price-supplier">
                                    <span class="editable-price" onclick="editSupplierPrice('${size}', 'pack', this)">£${p.pack.toFixed(2)}</span>
                                    <span class="qty-badge">(${p.packQty})</span>
                                </div>
                                <div class="price-sell">£${packSell.toFixed(2)} *</div>
                            </td>
                            <td class="price-cell">
                                <div class="price-supplier">
                                    <span class="editable-price" onclick="editSupplierPrice('${size}', 'single', this)">£${p.single.toFixed(2)}</span>
                                    <span class="qty-badge">(1)</span>
                                </div>
                                <div class="price-sell">£${singleSell.toFixed(2)} *</div>
                            </td>
                            <td class="stock-cell">${p.stock.toLocaleString()}</td>
                            <td class="stock-cell supplier">${p.supplierStock.toLocaleString()}</td>
                            <td class="watch-cell">
                                <label class="switch-small">
                                    <input type="checkbox" onchange="toggleSizeWatch('${size}', this.checked)">
                                    <span class="slider-small"></span>
                                </label>
                            </td>
                            <td class="due-cell">
                                <input type="text" class="due-date-input" placeholder="Awaiting Date" 
                                       onfocus="this.type='date'" onblur="if(!this.value)this.type='text'">
                            </td>
                            <td class="qty-cell">
                                <input type="number" class="qty-input" min="0" value="" 
                                       onchange="updateOrderQty('${size}', this.value)">
                            </td>
                        </tr>
                    `;
                }).join('')}
        ...
        <div class="pricing-legend" style="margin-top: 12px; display: flex; gap: 20px; flex-wrap: wrap; font-size: 13px; color: #6b7280;">
            <span><span class="editable-price" style="color: var(--info);">Blue prices</span> = Editable supplier costs</span>
            <span><span style="color: #dc2626;">Red prices *</span> = Your sell price (after ${productPricingState.markupPercent}% markup + volume discount)</span>
        </div>
        
        <div class="order-summary" style="margin-top: 16px; padding: 12px 16px; background: #f8fafc; border-radius: 8px; display: flex; justify-content: flex-end; gap: 30px; align-items: center;">
            <span>Order Lines: <strong id="orderLinesCount">0</strong></span>
            <span>Total Items: <strong id="totalItemsCount">0</strong></span>
            <span>Total: <strong id="orderTotal">£0.00</strong></span>
...
/**
 * Edit supplier price inline
 */
function editSupplierPrice(size, priceType, element) {
    const currentPrice = productPricingState.sizePrices[size][priceType];
    const input = document.createElement('input');
    input.type = 'number';
    input.className = 'inline-price-edit';
    input.value = currentPrice.toFixed(2);
    input.step = '0.01';
    input.min = '0';
    input.style.cssText = 'width: 70px; padding: 2px 4px; font-size: 13px; border: 1px solid var(--accent); border-radius: 4px;';
    ...
    const saveValue = () => {
        const newValue = parseFloat(input.value) || currentPrice;
        productPricingState.sizePrices[size][priceType] = newValue;
        recalculatePricingMatrix();
    };
 ...
 */
function updateGlobalMarkup(value) {
    productPricingState.markupPercent = parseFloat(value) || 50;
    recalculatePricingMatrix();
}
 ...
 */
function updateVolumeDiscountTable(value) {
    productPricingState.volumeDiscountTable = value;
    recalculatePricingMatrix();
}

/**
 * Update discount base only
 */
function updateDiscountBaseOnly(checked) {
    productPricingState.discountBaseOnly = checked;
    recalculatePricingMatrix();
}

/**
 * Toggle manual base price
 */
function toggleManualBasePrice(checked) {
    productPricingState.useManualBasePrice = checked;
    const group = document.getElementById('manualBasePriceGroup');
    if (group) {
...
/**
 * Update manual base price
 */
function updateManualBasePrice(value) {
    productPricingState.manualBasePrice = parseFloat(value) || 0;
    recalculatePricingMatrix();
}

/**
 * Recalculate and re-render pricing matrix
 */
function recalculatePricingMatrix() {
    const wrapper = document.querySelector('.size-pricing-table-wrapper');
    if (wrapper && currentProduct) {
        wrapper.innerHTML = renderSizePricingMatrix(currentProduct);
        updateOrderSummary();
    }
    
    // Also update the Price Breakdown section at the top
    updatePriceBreakdownSection();
}

/**
 * Update Price Breakdown section with current markup
 */
function updatePriceBreakdownSection() {
    if (!currentProduct) return;
    
    const supplierCost = currentProduct.priceRange?.min || currentProduct.minPrice || currentProduct.price || 8.60;
    const markupPercent = productPricingState.markupPercent;
    const markupAmount = (supplierCost * markupPercent / 100);
    const blankProductPrice = productPricingState.useManualBasePrice && productPricingState.manualBasePrice > 0
        ? productPricingState.manualBasePrice
        : (supplierCost + markupAmount);
    
    // Update the markup % display in Price Breakdown
    const markupLabel = document.querySelector('.price-breakdown-table .label-cell');
    if (markupLabel && markupLabel.textContent.includes('Supplier Product Markup')) {
        markupLabel.innerHTML = `Supplier Product Markup (${markupPercent}%)`;
    }
}

/**
 * Toggle watch for size
 */
function toggleSizeWatch(size, checked) {
    console.log(`Watch ${size}: ${checked}`);
}

/**
 * Update order quantity for size
 */
function updateOrderQty(size, qty) {
    productPricingState.sizePrices[size].orderQty = parseInt(qty) || 0;
    updateOrderSummary();
}
...
function updateOrderSummary() {
    ...
    sizes.forEach(size => {
        ...
        if (qty > 0) {
            orderLines++;
            totalItems += qty;
            // Use single price for simplicity
            const sellPrice = calculateMarkedUpPrice(p.single, qty);
            totalPrice += sellPrice * qty;
        }
    });
    
    const linesEl = document.getElementById('orderLinesCount');
    const itemsEl = document.getElementById('totalItemsCount');
    const totalEl = document.getElementById('orderTotal');
    
    if (linesEl) linesEl.textContent = orderLines;
...
function addToBasket() {
    alert('Added to basket! (Demo functionality)');
}
