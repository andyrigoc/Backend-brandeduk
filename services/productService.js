...
async function buildProductListQuery(filters, page, limit) {
  ...
  try {
    ...
    // Store custom display order for each style code
    ...
    queryResult.rows.forEach(row => {
      ...
      if (row.custom_display_order !== null && row.custom_display_order !== undefined) {
    ...
    const items = styleCodes.map(styleCode => {
      ...
      return {
        ...
        brand: product.brand || '',
        priceBreaks,
        displayOrder: displayOrderMap.get(product.code) || null
      };
