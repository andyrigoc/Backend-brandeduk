 ...
 * Supports Fixed Markup and Tiered Markup (persisted to API).
 */

const SUPPLIER_MARKUP_STORAGE_KEY = 'adminHub.supplierMarkupConfig';

// Import API_BASE from config.js (should be available globally)
const PRICING_RULES_API = typeof API_BASE !== 'undefined' ? `${API_BASE}/pricing-rules` : 'http://localhost:3004/api/pricing-rules';
...
function refreshMarkupUI() {
    ...
    if (fixed) {
        ...
        if (!fixed.dataset.bound) {
            fixed.dataset.bound = '1';
